...
16. Next wrap all of the lines excluding the `return` statement in a `for` loop, where we initialize some counter variable to `0`, continue while the counter variable is less than `MAX_REQUESTS`, and bump it up one each time.
