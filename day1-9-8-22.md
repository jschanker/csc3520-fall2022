...
2. Define a variable `bodyRequestData` whose value is in the above section (https://github.com/jschanker/csc3520-fall2022/blob/main/day1-9-8-22.md#:~:text=fun%20error%20message%3A-,Body%20data%20(Payload),-%3A%20Includes%20value%20of). Be sure to replace the `browserRefreshKey` to reference the variable that holds this value and remove the subject area CSC as we'll want to get the search results for every area. You might also want to replace `2022;FA` with a variable that holds this value, so it'll be easier to change in the future for other semesters. Look up [template literals](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals) to see a nice way to combine variable values and string literals without using string concatenation (`+`).
3. Now make a POST request using `axios`. This time you'll use `axios` (no `axios.post` or `axios.get`) and pass it an object with the **three** relevant properties:
  1. `url` is the request url, `method` is `"POST"`, 
  2. `data` will be `bodyRequestData`, and 
  3. `headers` which itself will be an object with properties `"content-type" : "multipart/form-data; boundary=----FormBoundary"` (the format of the request body, boundary is the separator between names of inputs and their values) and a property with a key of `cookie` whose value is 'ASP.NET_SessionId=' followed by whatever the session id is. 
This time, you'll use another `await` for the `axios` call and store it in `response`.
...
5. The next step is to parse the HTML as we did before to get the browser refresh key. Use the <code>parse</code> method of the 1nodeHTMLParser` object as you did before.
6. The relevant data is in the table. Go to Lions Den yourself and make a request for the course data and inspect the course table (using Inspect Element) to see if it has a unique id, so you could use `getElementById` as before. You can log its `innerHTML` property to see if you get the table of course data.
7. Next we'll want to get all of the table headings. This will make it so that we know which table cell corresponds to which piece of course data. This is important for making our scraper less fragile. For example, the third column might correspond to the course code now, but if columns are added or removed (as they have been in the past), it may not be that way in the future. To get all of the headings, you'll want to use <a href="https://www.npmjs.com/package/node-html-parser#user-content-htmlelementqueryselectorallselector">`querySelectorAll('th')`</a> which will give you all of the <code>&lt;th&gt;</code> tags belonging to the table as an `Array`.&nbsp; <strong>IMPORTANT NOTE</strong>: The POST request to the Lions Den to search for courses sometimes fails. In this case getting the element by id will return <code>null</code> so calling <code>querySelectorAll</code> on it will crash the program. To prevent this, you can use the <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Optional_chaining#try_it">optional chaining operator</a> by placing a question mark before each <code>.</code> when you're not sure if what comes before it will be <code>null</code> So, if you had an object referred to by <code>foo</code> that may or may not have a property <code>a</code>, you could use `foo.prop?.x` to "short-circuit" this property chain, resulting in `undefined` when there is no `prop` property on `foo` or whatever `foo.prop.x`, otherwise. The other option is to just put this in an `if` statement checking if `foo.prop != null` (this checks for `null` and `undefined`).
8. If you get a non-<code>null</code> value for the element, calling <code>querySelectorAll</code> here will give you an <code>Array</code> of all the cells. For each cell you'll want to get the text. So let's use the <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/map">`map`</a> method on the `Array` mapping each element to its <a href="https://github.com/taoqf/node-html-parser#htmlelementtext"><code>text</code></a> property.</li>
9. Check to see if you see the Array of table headings. Since we'll want to make each of these property keys, let's use snake case (spaces replaced with underscores, all lowercase letters). So, add to the output of the function we provide to `map` by replacing all spaces with underscores (<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/replaceAll&lt;code&gt;replaceAll">replaceAll</a> or for better browser compatibility use <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/replace#description">replace</a> with a RegEx with a <code>g</code> flag, with `g` standing for global). Then use <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/toLowerCase"><code>toLowerCase</code></a>. If you log the result, you should see an `Array` `['add', 'textbooks', 'course_code', 'name', 'type', 'faculty', 'seats_open', 'status', 'schedule',	'credits', 'begin_date', 'end_date']`. Store this result in a variable like `courseDataHeadings'. 
10. Next we'll want to get each row of course data. As before, you can use `querySelectorAll`, but this time use `tr` for the table rows. Then as before, use `map` on the resulting Array, this time mapping each row to an Array of all of its cells, again using `querySelectorAll` to get an Array of table data cells (`td`). This will yield an Array of Arrays of table data cells, one for each row. Then on this second `querySelectorAll`, call `map` again to get each cell's `text` property as before, calling [`trim`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/trim) on the result to remove leading and trailing whitespace. Now, if you log the result, you'll instead see the table data cells replaced with their (trimmed) text. Store this in a variable as well, say `courseDataRows`.
11. You'll notice there are some rows that don't contain any course data. When inspecting the table in the course search, you'll see that these are table rows with a class of `subItem` and only a single cell of data (spanning 12 columns):
```
<tr class=" subItem" style="display:none;">
			<td colspan="12">
	        <div style="width: 96%">
...
```
To exclude these from the list, we can add [`filter`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/filter) to the end, but we'll do this in the next step as we'll want to filter anything without a course code.

12. Now for each `courseDataRow`, we'll want to create an object where the keys are the column headings and the values are the table cells of course data at the same indices. For example, the table heading at index 2 is `course_code`, which will be the key for the item at index 2 of any table row, e.g., `CSC 352 01`. To accomplish this, we'll first use `map` on the `courseDataRows` Array. Then for each `Array` `courseDataRow`, we will map it to the result of [`reduce`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/reduce), which will be used to combine each `key-value` pair into a single object. The table looks something like the following:
```
acc                           | courseDataRow      | index | courseDataHeadings[index]
--------------------------------------------------------------------------------------
{}                            | ''                 | 0     | 'add'
--------------------------------------------------------------------------------------
{                             | ''                 | 1     | 'textbooks'
...
--------------------------------------------------------------------------------------
{                             | 'CSC 352 01'       | 2     | 'course_code'
 'add': '',                   |                    |       |
 'textbooks: '',              |                    |       |
}                             |                    |       |
--------------------------------------------------------------------------------------
{                             | 'Back-End WebDev'  | 3     | 'name'
 'add': '',                   |                    |       |
 'textbooks: '',              |                    |       |
 'course_code: 'CSC 352 01',  |                    |       |
}                             |                    |       |
--------------------------------------------------------------------------------------
```
So to use reduce here, you pass a function that takes three inputs, the first is the accumulator (denoted acc above), the second is the item from the Array (courseDataRow here), and the third is the `index`. You also pass a second argument that's the initial value of the accumulator (acc). After the initial accumulator, it will be the return value from the previous item. So, here the goal is to make the return value from each of the rows be the accumulator for the next row, so when we go through each `courseDataRow`, we will have an object with each property. (Note the last column is not one of the inputs here.) What this means is that say given an input `acc` object of `{'add': '', 'textbooks': ''}`, an input of `courseDataRow` 'CSC 352 01' and an input of index `2`, we want to get an output of `{'add': '', 'textbooks': '', course_code: 'CSC 352 01'}`. To do this we'd return an object with all properties from the current accumulator and the additional property with key `courseDataHeadings[index]` and `value` `courseDataRow`. You can do this in one line by performing a (shallow) copy of the `acc` object using spread syntax and adding a computed property object name (See https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Spread_syntax#spread_in_object_literals and https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment#computed_object_property_names_and_destructuring.)

13. You should get an Array of objects from the previous step, but as mentioned earlier, we'll want to remove the non-course rows. So, call `filter` on the result to only include the ones with a `courseCode` property (You're checking that the `courseCode` property is not an empty string, which is the only falsy string.) So you can just `return` the `courseCode` of the input here to remove all objects that don't have course codes. Store the result of this in a variable like `pageResults` (since it's only one of the pages).

14. Now we can do this for each page and do additional processing, but let's write this result in a file. To do this we'll use the built-in `fs` Node module (no intallation is necessary). `require` the `fs` module and store it in a variable `fs` at the top. Return `pageResults` from step 16 and then in the function passed to `then`, use the [`writeFileSync`](https://nodejs.org/api/fs.html#fswritefilesyncfile-data-options) function of `fs` to write the result of calling [JSON.stringify](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify) on the `result` in the file `course-data.json`. The `JSON.stringify` will serialize the object (convert it to a format that could be written to a file or transmitted as data) as a string in `JSON`, so we could deserialize it later with the [`JSON.parse`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse) method that'll convert it back to an object.

15. Now let's scrape every page of data. To start, first make sure that you use `let` (insetead of `const`) for your declaration of `browserRefreshKey` at the top of the `getCourseData` function definition as we'll need to get a new one for each page. Below this line declare and initialize the following additional variables:
  - `allResults`, an Array of all the page results, which should be initialized to an empty `Array`
  - `page`, which will be the requested page number (`1` is the second page of results, `2` is the third page, etc.). When you make a course search, no value is provided but using `0` will work as well. It'll probably be clearer to initialize this variable to `0` so you can bump up the counter with each subsequent page request instead of handling the case for the empty string separately.
  - `MAX_REQUESTS`, a constant represent the maximum number of attempts. This is necessary because a number of requests to get a page of course data will inexplicably fail. Setting this to say `1024` to get the say `64` pages of course data should be large enough, as it'd allow an average of `15` failed requests per page (`15 * 64` failed requests + 64 successful requests = `16 * 64 = 1024` total requests). Of course we could have more pages of course data in the future, but even with say 100 pages, it would allow over 9 failed requests per page on average. This variable is not strictly necessary as we could just keep going until we reach the last page, but we'll use this so that if the script were set to run automatically via a GitHub action to repeatedly scrape Lions Den for course updates, it would be programmed to stop after too many failed requests.
16. Next wrap all of the lines excluding the `let`/`const` statements for the variables and  `return` statement in a `for` loop, where we initialize some counter variable to `0`, continue while the counter variable is less than `MAX_REQUESTS`, and bump it up one each time. The variable declarations/initializations should be above the loop while the `return` statement should be below it. Some variables such as `bodyRequestData`, the one assigned to the `response` from the `POST` request, and `parseTree` will be declared and initialized in the `for loop`. Note that any variable declared (i.e., with `let` or `const`) inside the `for` loop will _not_ be available outside of it. Note it is possible to declare a variable with an assigment like `let foo;` outside of the loop and then set its value inside the loop (e.g., `foo = 42`. Then you will be able to access its value outside of the loop.   
17. Instead of `return`ing `pageResults`, we'll want to return the `Array` `allResults`. In the `for` loop, be sure to append all the items from `pageResults` to the `allResults` `Array`. To do this, you can use the Array's [push](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/push) method, which is used to add `1` or more items to the end of an Array. Because using `a.push(b)` will just add one item to `a` (e.g., if `a = [1, 2, 3]` and `b = [4, 5]`, then the Array `a` becomes `[[1, 2, 3], [4, 5]]` instead of `[1, 2, 3, 4, 5]`), you can use the spread syntax with `push` to instead [merge the two Arrays](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/push#merging_two_arrays). For this example, with two items, this would effectively be doing `a.push(b[0], b[1])`.
18. Modify your initialization of `bodyRequestData` (which should be inside the for loop so it gets updated!) to match what's sent when you get the pages of course results beyond the initial one. To figure out the changes you need, make a request to search all courses on Lions Den and then click on one of the letter ranges on the bottom while Developer Tools is open. Then check the Network tab. You'll notice two differences:
    - Not all name-value pairs are sent in the body of the request data. (The ones after the semester are no longer included.) So, in the template literal include a [placeholder](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals#:~:text=other%20parts%20called-,placeholders,-%2C%20which%20are%20embedded) for an expression with the [conditional operator](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Conditional_Operator) which uses the template literal with this remaining part of the request data when `page` is not `0` (i.e., `page` is truthy) and the empty string otherwise.
    - The subsequent request makes use of the `page` value. To figure out where to place this value, see which name is associated with a number such as `42` (if you clicked the `43`rd page of results.)
![image](https://user-images.githubusercontent.com/9159008/192122765-36f198ed-1206-48cd-bfeb-a9de2eb310c0.png)
19. After getting the parse tree from the response, call `getBrowserRefreshKey` passing this parse tree as an input and _reassign_ `browserRefreshKey` to be the `return`ed value. This will be used so you can make the `POST` request for the next page since browser refresh keys can only be used once. Add a `console.log` statement indicating the current page and `browseRefreshKey`. This will be used for you to monitor the progress of the execution of the script and how many successful/failed requests there are.
20. If you didn't do this already, make sure you have an `if` statement surrounding the code where you get `courseDataHeadings` and `courseDataRows`. The condition should be that the element with the id of the course data table isn't `null` (i.e., that the request for the page of results was successful). In the body of that `if` statement, bump up the `page` variable. (You only want to go to the next page if you were able to get the results for page `page`.) 
21. Finally check to see if you're on the last page and if so [`break`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/break) from the loop. Again, the number of pages can change so you shouldn't assume that `63` is the last page number. To see if you're on the last page, figure out what's different in the HTML between the last page and the non-last pages. You may want to use [`querySelector`](https://developer.mozilla.org/en-US/docs/Web/API/Document/querySelector) (not `querySelectorAll` since there will only be one), passing it the selector for a class (how do you select all elements belonging to a class in CSS?) and check to see if the value of its `text` property [includes](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/includes) certain text. 
