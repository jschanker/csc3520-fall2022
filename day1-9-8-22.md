...
3. Now go back to the orgininal function. Instead of `return`ing the session id string, `return` an object literal with a key of `sessionCookie` and the value you were previously returning and a key of `browserRefreshKey` and the value of calling this `getBrowserRefreshKey` on `parseTree`.
